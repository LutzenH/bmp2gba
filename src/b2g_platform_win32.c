...
int internal_platform_list_files_within_folder(cvec_str* folders, const char* folder, int recursive, const char* filter_by_extension, const char* prefix)
{
	...
	if (find_handle != INVALID_HANDLE_VALUE) {
		do {
			...
			else if (!(fd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) && (filter_by_extension == NULL || internal_platform_helper_string_ends_with(fd.cFileName, filter_by_extension))) {
